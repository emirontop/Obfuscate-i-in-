...
-- Yeni Tab metatable'ını ekleyin (Window tanımından önce)
local Tab = {}
Tab.__index = Tab

function Tab:AddSection(name, options)
    assert(type(name) == "string", "Section name must be string")
    local section = Section:new(self.SectionCanvas, self.ParentWindow, name, options)
...
function Window:AddTab(name)
    ...
    -- Tab nesnesi
        ...
        Sections = {},
        ParentWindow = self  -- Parent window referansı ekleyin
    }, Tab)
